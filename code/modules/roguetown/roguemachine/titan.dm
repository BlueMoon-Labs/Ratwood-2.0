...
/proc/make_outlaw(raw_message)
	if(raw_message in GLOB.outlawed_players)
		...
		priority_announce("[raw_message] больше не разыскивается в королевстве.", "[SSticker.rulertype] издаёт указ", 'sound/misc/royal_decree.ogg', "Captain")
		return FALSE
	var/found = FALSE
	...
	if(!found)
		return FALSE
	GLOB.outlawed_players += raw_message
	priority_announce("[raw_message] объявлен вне закона и должен быть схвачен или убит.", "[SSticker.rulertype] издаёт указ", 'sound/misc/royal_decree2.ogg', "Captain")
	return TRUE
...
/proc/purge_laws()
	...
	priority_announce("Все законы земли уничтожены!", "ЗАКОНЫ УНИЧТОЖЕНЫ", 'sound/misc/lawspurged.ogg', "Captain")

/proc/purge_decrees()
	GLOB.lord_decrees = list()
	priority_announce("Все прежние указы земли уничтожены!", "УКАЗЫ УНИЧТОЖЕНЫ", pick('sound/misc/royal_decree.ogg', 'sound/misc/royal_decree2.ogg'), "Captain")

/proc/become_regent(mob/living/carbon/human/H)
	priority_announce("[H.name], [H.get_role_title()], становится регентом королевства.", "Новый регент", pick('sound/misc/royal_decree.ogg', 'sound/misc/royal_decree2.ogg'), "Captain")
