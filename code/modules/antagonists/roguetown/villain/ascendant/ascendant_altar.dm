...
/obj/structure/ascendant_altar/proc/ascend(mob/living/user)
	...
	switch(ascend_stage)
		if(1)
			...
			priority_announce("Лей-линии содрогаются в неестественном извращении — СТАРШИЙ АРКАН: ШУТ, ПРЯМОЙ.", "СНОВИДЕЦ", 'sound/villain/dreamer_warning.ogg')
		if(2)
			...
			priority_announce("Небо начинает вращаться быстрее — СТАРШИЙ АРКАН: ПОВЕШЕННЫЙ, ПЕРЕВЁРНУТЫЙ", "СНОВИДЕЦ", 'sound/villain/dreamer_warning.ogg')
		if(3)
...
/obj/structure/ascendant_altar/proc/heavensaysdanger()
	priority_announce("СНОВИДЕЦ ВОЗНЁССЯ — СТАРШИЙ АРКАН: БАШНЯ, ПЕРЕВЁРНУТАЯ", "БОГ ИДЁТ", 'sound/villain/ascendant_intro.ogg')
	sleep(15 SECONDS)
